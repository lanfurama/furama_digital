...
def save_review_data(result, crawler):
    # Kiểm tra nếu review đã tồn tại dựa trên url, source và ngày tạo
    today = datetime.today().date()
    
    # Tìm review đã có với điều kiện url, source và ngày tạo = today
    review = OtaReview.objects.filter(url=result["url"], source=result["source"], created_at__date=today).first()
    
    if review:
        # Nếu review đã tồn tại và là trong ngày hôm nay, không cần insert nữa
        return {
            "resort": result["resort"],
            "url": result["url"],
            "source": result["source"],
